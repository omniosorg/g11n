...
#pragma ident	"@(#)lcl_convert.c	1.3	00/01/06 SMI"
		...
		while(1){
			...
			if(ret == -1){				
			  iconv_close(i_conv);
				...
				/* Bug fix 4073017/4073484 */
				else if ((lerrno == EILSEQ) && (bad_charset_flag)){  
					errno=0;
					repeat=1;
					*to_ptr++='?';
				        to_bytes--;
...
_lct_get_convert(LCTd lctd, LctNEAttribute to_form, LctNEAttribute attr, LclCharsetSegmentSet **to_segs)
{
	...
	char	*body_encoding,*mail_charset;
	...
	/* Ciaran Section */
	/*****************/
	if((lctd->contents->type == LctNMsgText) && (lctd->contents->body)){
           mail_type_str = _lct_get_string_attribute(lctd, to_form, 
			   LctNMailType);
           mail_type = _lcl_str_to_mailtype(mail_type_str);

	   body_encoding = _lct_get_string_attribute(lctd, from_form,
			   LctNBodyEncoding);
	   body_encoding_type = _lcl_str_to_mailencoding(lctd, 
				body_encoding, 0, (char *)NULL);
	   mail_charset = (char *)_lct_get_mime_charsetname(lctd, 
			  to_charset);
	   from_size = strlen(from_buf);
	}
	/*****************/
