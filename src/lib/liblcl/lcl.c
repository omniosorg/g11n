...
#pragma ident	"@(#)lcl.c	1.4	00/01/07 SMI"
...
lcl_open(char *locale)
{
	...
	/* Order of Parsing should not be changed as Form Parsing
	   depends on Charset Info Parsing - Prabhat */
	...
	if (_LclParseFormInfo(lcld, &(lcld->form_info)) < 0) {
	  lcl_close(lcld);
	  return (LCLd)NULL;
	}
