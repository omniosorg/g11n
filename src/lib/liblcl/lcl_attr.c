...
#pragma ident	"@(#)lcl_attr.c	1.2	00/01/06 SMI"

#include "lcl.h"
#include "lcl_internal.h"

/* bug fix 4073017/4073484 flag is checked in lcl_convert.c _exec_iconv*/
extern int bad_charset_flag;
...
_lcl_copy_charset_linked_list(LclCharsetLinkedList *list)
{
	...
	while(ref_ptr){
		...
		new_ptr->mimeInfo = ref_ptr->mimeInfo;
...
_lcl_destroy_charset_linked_list(LclCharsetLinkedList	*list)
{
	...
	while(ptr){
		...
		if(old_ptr->name)
			free(old_ptr->name);
		/* For FormAttr - user should free LclCteAtom */
		free(old_ptr);
	}
}
...
set_mail_string_attribute(LCTd lctd, LctNEAttribute form, LctNEAttribute attr, char *str)
{
	...
	if(charset){
		...
		/* bug fix 4073017/4073484 */
		/* if a string was passed to set the charset value then if the string is invalid 
 		   set the bad_charset_flag */
		if ((str) && (*str != 'X'))
		   bad_charset_flag=1;
		return -1;
	      }
