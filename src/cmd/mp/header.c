...
char **dest ;
{
  if (hdr_equal(hdr))
    {
      *dest = (char *)malloc((unsigned) (strlen(nextline) - strlen(hdr) + 1)) ;
  ...
  if (hdr_equal(hdr))
    {
      ...
      while (i < MAXCONT && !emptyline(nextline) && isspace(nextline[0]))
        {
          dest[i] = (char *)malloc((unsigned) (strlen(nextline) + 1)) ;
...
int digest ;            /* Parsing digest headers */
{
  ...
  char tmpstr[MAXLINE] ;
	...
	if(!(hdr_equal(X_UNIX_FROM) ||\
		hdr_equal(FROMHDR) || hdr_equal(FROM_HDR) ||\
		hdr_equal(APP_FROMHDR) || hdr_equal(TOHDR) ||\
		hdr_equal(APP_TOHDR) || hdr_equal(CCHDR) ||\
		hdr_equal(SUBJECTHDR) || hdr_equal(DATEHDR) ||\
		hdr_equal(NEWSGROUPSHDR) || hdr_equal(NEWSGROUPHDR) ||\
		hdr_equal(REPLYHDR) || hdr_equal(CONTENT_LEN))) {
		doc_type = DO_TEXT ;
		return ;
	}
