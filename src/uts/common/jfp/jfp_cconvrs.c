...
#pragma ident  "@(#)jfp_cconvrs.c	1.16     04/08/24 SMI"

/*
 * JFP Code Converter for Shift-JIS terminal (Upper side).
 */
...
extern struct streamtab jconvrsinfo;
 ...
 */
static int jconvrsopen(queue_t *, dev_t *, int, int, cred_t *);
static int jconvrsclose(queue_t *, int, cred_t *);
static int jconvrswput(queue_t *, mblk_t *);
static int jconvrsrput(queue_t *, mblk_t *);
...
static struct module_info jconvrsmiinfo = {
	113,
	"jconvrs",
	0,
	512,
...
static struct qinit jconvrsrinit = {
	jconvrsrput,
	NULL,
	jconvrsopen,
	jconvrsclose,
...
static struct module_info jconvrsmoinfo = {
	113,
	"jconvrs",
	0,
	512,
...
static struct qinit jconvrswinit = {
	jconvrswput,
	NULL,
	jconvrsopen,
	jconvrsclose,
...
/*ARGSUSED*/
int
jconvrsopen(queue_t *q, dev_t *dev, int oflag, int sflag, cred_t *cred_p)
{
	register rsjis_state_t *sp;
	...
	/* Memory allocation */
	sp = (rsjis_state_t *)
		kmem_zalloc(sizeof (rsjis_state_t), KM_NOSLEEP);
	if (sp == (rsjis_state_t *)NULL)
		return (ENOMEM);
	mcp = allocb(sizeof (mc_m_ctl_msg_t), BPRI_MED);
	if (mcp == (mblk_t *)NULL) {
		kmem_free((void *)sp, sizeof (rsjis_state_t));
		return (ENOMEM);
	}
...
/*ARGSUSED*/
static int
jconvrsclose(queue_t *q, int flag, cred_t *cred_p)
{
	register rsjis_state_t *sp = (rsjis_state_t *)q->q_ptr;

	qprocsoff(q);
	kmem_free(sp, sizeof (rsjis_state_t));
	q->q_ptr = NULL;
	return (0);
}
 ...
 */
int
jconvrswput(queue_t *q, mblk_t *mp)
{
	register rsjis_state_t *sp = (rsjis_state_t *)q->q_ptr;
	mblk_t *nmp = NULL;
	int ret_val;
 ...
 */
static int
jconvrsrput(queue_t *q, mblk_t *mp)
{
	register rsjis_state_t *sp = (rsjis_state_t *)q->q_ptr;
	int ret_val;
	...
	case M_CTL:
	{
		...
		if (mctlp->mc_m_ctl_cmd == MC_PEERQUERY) {
			...
			if (sp->do_conv != ON) {
				cmn_err(CE_WARN,
				"jconvrs: Confuse conversion status.\n");
				return (-1);
			} else {
				return (0);
			}
...
do_ioctl(queue_t *q, mblk_t *mp)
{
	...
	sp = (rsjis_state_t *)q->q_ptr;

	switch (iocp->ioc_cmd) {
	case TIOCSTI:
		ret_val = sj2euc(mp->b_cont, &nmp, &(sp->w_backup));
		if (ret_val != 0) {
			...
			if (nmp) {
				 ...
				 */
#ifdef DBG_W
				cmn_err(CE_NOTE, "rswput(): TIOCSTI converted");
#endif
	...
	case EUC_OXLON:
		...
		if (sp->notify) {
			mcp = allocb(sizeof (mc_m_ctl_msg_t), BPRI_MED);
			if (mcp == (mblk_t *)NULL) {
				cmn_err(CE_NOTE, "jconvrs: No blocks.");
				return;
			}
			mcp->b_datap->db_type = M_CTL;
			mcp->b_wptr = mcp->b_rptr + sizeof (mc_m_ctl_msg_t);
